...
-- =====================================================================
-- DVLD schema converted from SQL Server to PostgreSQL
-- SQL Server is the single source of truth for schema, fields,
-- constraints, relationships, defaults, and views.
--
...
--
-- All PostgreSQL table, column, constraint, and view identifiers are
-- explicitly written in lowercase.
-- Idempotent: CREATE TABLE IF NOT EXISTS / CREATE OR REPLACE VIEW.
-- NOTE: the SSMS source spells the people column "Gendor"; this schema
-- keeps "gender" because the DAL (clsSqlPeoples) writes to "gender".
-- applicationtypes
CREATE TABLE IF NOT EXISTS applicationtypes (
    applicationtypeid int GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS licenseclasses (
    licenseclassid int GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS testtypes (
    testtypeid int GENERATED BY DEFAULT AS IDENTITY,
...
-- SQL Server SSMS artifact table; retained for schema parity.
CREATE TABLE IF NOT EXISTS sysdiagrams (
    name varchar(128) NOT NULL,
    principal_id integer NOT NULL,
    diagram_id integer GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS sysdiagrams (
    name varchar(128) NOT NULL,
    principal_id integer NOT NULL,
    diagram_id int GENERATED BY DEFAULT AS IDENTITY,
    version integer NULL,
    definition bytea NULL,
    CONSTRAINT pk_sysdiagrams PRIMARY KEY (diagram_id),
    CONSTRAINT uk_principal_name UNIQUE (principal_id, name)
...
CREATE TABLE IF NOT EXISTS drivers (
    driverid int GENERATED BY DEFAULT AS IDENTITY,
    personid int NOT NULL,
    createdbyuserid int NOT NULL,
...
CREATE TABLE IF NOT EXISTS licenses (
    licenseid int GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS localdrivinglicenseapplications (
    localdrivinglicenseapplicationid int GENERATED BY DEFAULT AS IDENTITY,
    applicationid int NOT NULL,
...
CREATE TABLE IF NOT EXISTS testappointments (
    testappointmentid int GENERATED BY DEFAULT AS IDENTITY,
    ...
    retaketestapplicationid integer NULL,
...
CREATE TABLE IF NOT EXISTS tests (
    testid int GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS detainedlicenses (
    detainid int GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE IF NOT EXISTS internationallicenses (
    internationallicenseid int GENERATED BY DEFAULT AS IDENTITY,
    applicationid int NOT NULL,
    driverid int NOT NULL,
...
--
-- CREATE TABLE IF NOT EXISTS leaves an existing table and its identity
-- sequence untouched.  If the table was previously populated by a source
-- that did not advance the identity sequence (explicit-ID inserts under
-- an older schema, migration, dump/restore), the sequence can sit below
-- MAX(id).  The next auto-generated INSERT would then collide with an
-- existing primary key and fail with 23505 "duplicate key value violates
-- unique constraint".  Running setval BEFORE any seed INSERT guarantees
-- the next generated ID is always MAX(id) + 1, so re-running this script
-- is safe against pre-existing rows.
-- =====================================================================
SELECT setval(pg_get_serial_sequence('applicationtypes', 'applicationtypeid'),
              (SELECT COALESCE(MAX(applicationtypeid), 0) + 1 FROM applicationtypes), false);
SELECT setval(pg_get_serial_sequence('countries', 'countryid'),
              (SELECT COALESCE(MAX(countryid), 0) + 1 FROM countries), false);
SELECT setval(pg_get_serial_sequence('licenseclasses', 'licenseclassid'),
              (SELECT COALESCE(MAX(licenseclassid), 0) + 1 FROM licenseclasses), false);
SELECT setval(pg_get_serial_sequence('testtypes', 'testtypeid'),
              (SELECT COALESCE(MAX(testtypeid), 0) + 1 FROM testtypes), false);
SELECT setval(pg_get_serial_sequence('people', 'personid'),
              (SELECT COALESCE(MAX(personid), 0) + 1 FROM people), false);
SELECT setval(pg_get_serial_sequence('users', 'userid'),
              (SELECT COALESCE(MAX(userid), 0) + 1 FROM users), false);
SELECT setval(pg_get_serial_sequence('applications', 'applicationid'),
              (SELECT COALESCE(MAX(applicationid), 0) + 1 FROM applications), false);
SELECT setval(pg_get_serial_sequence('drivers', 'driverid'),
              (SELECT COALESCE(MAX(driverid), 0) + 1 FROM drivers), false);
SELECT setval(pg_get_serial_sequence('licenses', 'licenseid'),
              (SELECT COALESCE(MAX(licenseid), 0) + 1 FROM licenses), false);
SELECT setval(pg_get_serial_sequence('localdrivinglicenseapplications', 'localdrivinglicenseapplicationid'),
              (SELECT COALESCE(MAX(localdrivinglicenseapplicationid), 0) + 1 FROM localdrivinglicenseapplications), false);
SELECT setval(pg_get_serial_sequence('testappointments', 'testappointmentid'),
              (SELECT COALESCE(MAX(testappointmentid), 0) + 1 FROM testappointments), false);
SELECT setval(pg_get_serial_sequence('tests', 'testid'),
...
CREATE OR REPLACE VIEW detainedlicenses_view AS
SELECT
    dl.detainid,
    dl.licenseid,
    dl.detaindate,
...
RIGHT OUTER JOIN detainedlicenses dl ON l.licenseid = dl.licenseid;
...
CREATE OR REPLACE VIEW localdrivinglicensefullapplications AS
SELECT
    a.applicationid,
    a.applicantpersonid,
    a.applicationdate,
